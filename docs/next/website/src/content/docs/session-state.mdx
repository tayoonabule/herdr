...
| --- | --- | --- |
| Jcode | Built in | `jcode --resume <id>` |
...
Jcode reports its native session identity directly when it detects Herdr, so it needs no separate Herdr integration install. For other agents, run `herdr integration status` to check installed integration versions and reinstall outdated integrations with `herdr integration install <agent>`.
