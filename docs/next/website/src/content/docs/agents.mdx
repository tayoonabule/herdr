...
| --- | --- | --- |
| Jcode | screen manifest | built-in native session bridge |
...
Jcode needs no separate `herdr integration install` step. Current Jcode releases detect Herdr automatically, report their native session ID as `herdr:jcode`, and resume with `jcode --resume <id>`. Herdr keeps screen detection authoritative for state so its live prompt, spinner, and interactive login waits remain visible without overriding Jcode's own lifecycle configuration.
