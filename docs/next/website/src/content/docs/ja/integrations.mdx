...
Jcode には Herdr セッションブリッジが組み込まれています。`herdr integration install` は不要です。Jcode は再開可能なセッション ID を `herdr:jcode` として報告し、Herdr は `jcode --resume <id>` で再開します。ライフサイクル状態の権威は引き続きスクリーンマニフェスト検出です。`jcode` が `PATH` にある場合、設定のインテグレーションタブにはインストール済みとして表示されます。
...
| セッション識別 | Jcode、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Qwen Code、Cursor Agent CLI、Hermes Agent、Antigravity CLI、Grok CLI | 組み込みブリッジまたはインストール済みインテグレーションが復元用のネイティブセッション参照を報告します。状態は引き続き Herdr のスクリーンマニフェスト検出から得られます。 |
