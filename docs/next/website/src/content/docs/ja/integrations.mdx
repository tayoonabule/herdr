...
Jcode には Herdr セッションブリッジが組み込まれています。`herdr integration install` は不要です。Jcode は再開可能なセッション ID を `herdr:jcode` として報告し、Herdr は `jcode --resume <id>` で再開します。ライフサイクル状態の権威は引き続きスクリーンマニフェスト検出です。
