...
Jcode 检测到 Herdr 后会直接上报原生会话 ID,因此无需单独安装 Herdr 集成。对于其他智能体,运行 `herdr integration status` 查看已安装的集成版本,并用 `herdr integration install <agent>` 重新安装过期的集成。
