...
Jcode 内置 Herdr 会话桥接,无需运行 `herdr integration install`。Jcode 以 `herdr:jcode` 上报可恢复会话 ID,Herdr 使用 `jcode --resume <id>` 恢复会话,生命周期状态仍由屏幕清单检测决定。当 `jcode` 位于 `PATH` 中时,设置的集成选项卡会将其显示为已安装。
...
| 会话身份 | Jcode、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Qwen Code、Cursor Agent CLI、Hermes Agent、Antigravity CLI、Grok CLI | 内置桥接或已安装的集成上报用于恢复的原生会话引用。状态仍来自 Herdr 的屏幕清单检测。 |
...
一些集成和内置桥接会上报智能体的原生会话引用。除非被 `[session] resume_agents_on_restore = false` 禁用,Herdr 会在服务器重启后使用官方会话引用恢复 Jcode、Claude Code、Codex、Devin CLI、Droid、Kimi Code CLI、Qoder CLI、Qwen Code、Cursor Agent CLI、Grok CLI、GitHub Copilot CLI、Pi、OMP、Hermes Agent、OpenCode、Kilo Code CLI、MastraCode 和 Antigravity CLI 的窗格。

Jcode 会话恢复为内置功能。其他原生会话恢复需要最新的 Herdr 集成: Pi 集成版本 `2`、OMP 版本 `3`、Claude Code 版本 `6`、Codex 版本 `5`、GitHub Copilot CLI 版本 `2`、Devin CLI 版本 `2`、Droid 版本 `2`、Kimi Code CLI 版本 `3`、Qoder CLI 版本 `2`、Qwen Code 版本 `1`、Cursor Agent CLI 版本 `1`、Grok CLI 版本 `1`、OpenCode 版本 `5`、Kilo Code CLI 版本 `1`、Hermes Agent 版本 `5`、MastraCode 版本 `1`、Antigravity CLI 版本 `1`。用 `herdr integration status` 查看已安装版本。
