...
Jcode 不需要单独运行 `herdr integration install`。当前 Jcode 版本会自动检测 Herdr,以 `herdr:jcode` 上报原生会话 ID,并通过 `jcode --resume <id>` 恢复。Herdr 继续以屏幕检测作为状态权威,因此可以显示 Jcode 的实时提示符、活动指示器和交互式登录等待,同时不会覆盖 Jcode 自身的生命周期配置。
