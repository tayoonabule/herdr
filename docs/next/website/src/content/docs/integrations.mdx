...
Jcode includes its Herdr session bridge. No `herdr integration install` command is needed: Jcode reports its resumable session ID as `herdr:jcode`, Herdr resumes it with `jcode --resume <id>`, and screen-manifest detection remains the lifecycle-state authority.
