...
Jcode includes its Herdr session bridge. No `herdr integration install` command is needed: Jcode reports its resumable session ID as `herdr:jcode`, Herdr resumes it with `jcode --resume <id>`, and screen-manifest detection remains the lifecycle-state authority. When `jcode` is on your `PATH`, the Settings integrations tab shows it as installed.
...
| Session identity | Jcode, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI | The built-in bridge or installed integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
